...
def getLinks(url):
    ...
    for i, url in enumerate(page):
        ...
        #Code added for getting the contents of web data

        response = urllib2.urlopen(url) 
        webContent = response.read()
        
        #x = re.findall('\S+infosys\S+', webContent)
        #if len(x) > 0 :
         #   print x
        m = re.search(r'((\w+\W+){0,4}infosys(\W+\w+){0,4})', webContent)
